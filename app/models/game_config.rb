class GameConfig < ActiveRecord::Base
  ...
  def self.method_missing(method, *args)
    ...
    if method_name =~ /=$/
      ...
      if item = find_by_key(var_name)
        ...
        GameConfig.create(key: var_name, value: value, description: var_name)
      end
    else
      Rails.cache.fetch("game_config:#{method}") do
  ...
  def update_cache
    Rails.cache.write("game_config:#{self.key}", self.value)
  end
